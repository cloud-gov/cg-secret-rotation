...
if [ "${GENERATE_POSTGRES_PASSPHRASE:-}" == "true" ]; then
  spruce json secrets-updated/secrets.yml \
    | jq --arg password "$(cat /dev/urandom | LC_ALL=C tr -dc "a-zA-Z0-9" | head -c 32)" '.secrets.bosh_postgres_password = $password' \
    | spruce merge \
    > secrets-updated/tmp.yml
  mv secrets-updated/tmp.yml secrets-updated/secrets.yml
fi

if [ "${GENERATE_MBUS_PASSPHRASE:-}" == "true" ]; then
  spruce json secrets-updated/secrets.yml \
    | jq --arg password "$(cat /dev/urandom | LC_ALL=C tr -dc "a-zA-Z0-9" | head -c 32)" '.secrets.bosh_mbus_password = $password' \
    | spruce merge \
    > secrets-updated/tmp.yml
  mv secrets-updated/tmp.yml secrets-updated/secrets.yml
fi

if [ "${GENERATE_VCAP_PASSPHRASE:-}" == "true" ]; then
  vcap_password=$(cat /dev/urandom | LC_ALL=C tr -dc "a-zA-Z0-9" | head -c 32)
  vcap_password_hash=$(echo "${vcap_password}" | mkpasswd -s -m sha-512)
  spruce json secrets-updated/secrets.yml \
    | jq --arg vcap_password "${vcap_password}" --arg vcap_password_hash "${vcap_password_hash}" \
      '.secrets.bosh_vcap_password = $vcap_password_hash | .secrets.bosh_vcap_password_plain = $vcap_password' \
    | spruce merge \
    > secrets-updated/tmp.yml
  mv secrets-updated/tmp.yml secrets-updated/secrets.yml
fi
